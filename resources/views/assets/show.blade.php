                ...
                @else
                    <form action="{{ route('trade.execute') }}" method="POST" class="space-y-4"
                         x-data="{ 
                             ...
                             dailyLimit: {{ (float)(auth()->user()->alpaca_is_paper ? (auth()->user()->daily_spend_limit ?? 5000.0) : (auth()->user()->live_daily_spend_limit ?? 5000.0)) }},
                             spentToday: {{ (float)auth()->user()->getDailySpent((bool)auth()->user()->alpaca_is_paper) }},
                        ...
                        <div class="space-y-2">
                            ...
                            <div x-show="estimatedCost > (dailyLimit - spentToday)" class="p-3.5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-[11px] leading-normal font-bold">
                                ⚠️ Esta compra excede tu límite diario disponible. Te quedan <span x-text="'$' + Math.max(0, dailyLimit - spentToday).toFixed(2)"></span> de un límite total de <span x-text="'$' + dailyLimit.toFixed(2)"></span>.
                            </div>
                        </div>
